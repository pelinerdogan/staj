...
public class allianzcontroller {
	...
	public campaign getcampaignbyID(@PathVariable("id") String id) { // Belirli idli kampanyayı döndürüyor
		long start = System.currentTimeMillis();
		campaign l = allianzservice.getWithid(id);
		long end = System.currentTimeMillis();
		if(end-start>5)
			System.out.println("işlem " + (end-start) + "Milisaniye sürdü");
		return l;
	...
	public  ResponseEntity<Map<String, Integer>> campaignstatistics() { // kampanya istatistiklerini String olarak "Active : -aktif sayısı- , Deactive : -deactive sayısı-
		long start = System.currentTimeMillis();
		 ResponseEntity<Map<String, Integer>> s = allianzservice.getCampaignStatistics();
		long end = System.currentTimeMillis();
		if(end-start>5)
			System.out.println("işlem " + (end-start) + "Milisaniye sürdü");
		return s;
